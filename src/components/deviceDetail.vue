	...
	<div class="fullDiv" style="background: #eee; font-size: 0.28rem;">
		<div class="deviceDetail-content">
			<div class="deviceDetail-top">
				<div class="device-status" v-if="curDeviceDetail.status==1">正常</div>
				<div class="device-status error" v-if="curDeviceDetail.status==2">异常</div>
				<div class="device-message1">
					<p class="device-number">设备号：{{curDeviceDetail.sn}}</p>
					<p class="device-site">站点：{{curDeviceDetail.stationName}}</p>
				</div>
			</div>
			<div class="deviceDetail-bottom">
				<p class="device-tit">设备运行</p>
				<ul class="abnormal-list">
					<li v-for="(item,index) in curDeviceDetail.warning_list" :key="index">
				...
				<ul class="device-message2">
					<li class="device-address" @click="goMap">{{curDeviceDetail.address}}</li>
					<li class="device-operate" @click="callPhone(curDeviceDetail.mobile)">{{curDeviceDetail.managerName}}<span class="phone">{{curDeviceDetail.mobile}}</span></li>
				</ul>
			</div>
...
export default {
    ...
    methods: {
    	goMap() {
    		var lat=this.curDeviceDetail.latitude
    		var lng=this.curDeviceDetail.longitude
    	...
    	opendoorConfirm(e) {
    		...
    		MessageBox.confirm('确定执行远程开门?').then(action => {
				...
				this.$store.dispatch('remoteOperate',qs.stringify({
					device_id: this.curDeviceDetail.id,
					clevel: 'OPENDOOR'
				}))
	    		setTimeout(function(){
	...
	//		if(this.curDeviceDetail.id==id){
	//			return
	//		}
	        this.$store.dispatch('getDeviceDetail',{
	        	params:{
				    station_id:id
				}
	        })
