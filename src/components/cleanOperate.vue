<template>
	<div class="fullDiv" style="background: #eee;">
		<div class="feedOperate-content">
			...
			<div class="feed-name-list clear">
				<div class="clean-name-item" >
					<mt-checklist
				      ...
				      :options="cleanPositionList">
				    </mt-checklist>
					<!--<Checkbox-group v-model="checkedGroup">
				        <Checkbox v-for="(item, index) in cleanPositionList" :key="index" :label="item.id">{{item.value}}</Checkbox>   
				    </Checkbox-group>-->
				</div>
				<!--{{checkedGroup}}-->
			</div>
			...
			<div class="btn-feed-confirm" @click="addCleanConfirm">确认</div>
			<Modal
...
//      },
		...
		addCleanConfirm() {
			...
			var clean_detail=''
			var self=this
			this.cleanPositionList.forEach(function(item){
				var clean_status=false
				self.checkedGroup.forEach(function(value){
					if(item.id==value){
	...
	created() {
		...
		if(this.cleanPositionList.length==0){
			this.$store.dispatch('getCleanPositionList')
		}
