		...
		<div class="deviceManagement-content page-content">
			...
			<div class="feed-list">
				<scroller 
					...
					<ul>
						<li class="feed-item" v-for="(item, index) in addMaterielList" :key="index">
							<a :href="'#feedDetail/'+item.id">
								<p class="feed-tit">
									<span class="feed-site">{{item.stationName}}</span>
									<span class="feed-person" style="text-align: center;">{{item.name}}</span>
									<span class="feed-time" style="text-align: right;">{{item.date}}</span>
								</p>
	   	...
	   	getAddMaterielList() {
	   		api.getAddMaterielList({
	       		params:{
	            	...
	            	this.$store.commit('get_add_materiel_list', res)
	            	this.page++   
